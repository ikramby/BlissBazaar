...
const EditProductForm = ({ productId, name, description, imageUrl, price, category, color, manufacturer, onSale }) => {
  ...
  useEffect(() => {
    ...
    if (paramProductId) {
      const fetchProduct = async () => {
        try {
          const response = await axios.get(`http://localhost:7000/products/${paramProductId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:7000/products/${paramProductId}`, updatedProduct);
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      >
        ...
        <CardMedia sx={{ height: 500, width: '100%' }} image={imageUrlInput} title={name} />
        <TextField
          label="Image URL"
          variant="outlined"
          value={imageUrlInput}
